...
from XPLMPlugin import XPLM_MSG_WILL_WRITE_PREFS, XPLMFindPluginBySignature, XPLMSendMessageToPlugin
...
class PythonInterface(checkBase):
    ...
    def XPluginStart(self):
        self.Sig = "xppython3.DataAccess.ShareData"
    ...
    def XPluginEnable(self):
        ...
        self.log("Got plugin id: {}".format(dataRefEditor))
        for t in self.types:
            # a) Find a data ref of that type,
            self.log("Testing {}".format(t['refCon']))
            t['dataRefName'] = 'xppython3/test/{}'.format(t['refCon'])
            ...
            XPLMSendMessageToPlugin(dataRefEditor, 0x01000000, t['dataRefName'])
            self.log("Sending for {}".format(t['dataRefName']))
