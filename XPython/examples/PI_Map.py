...
from XPLMMap import xplm_MapLayer_Fill, xplm_MapLayer_Markings, xplm_MapOrientation_Map, xplm_MapOrientation_UI
from XPLMMap import XPLM_MAP_USER_INTERFACE, XPLM_MAP_IOS
...
class PythonInterface():
    ...
    def XPluginStop(self):
        self.log("stopping")
        if self.g_layer:
            self.log("Destroying map layer")
            XPLMDestroyMapLayer(self.g_layer)

    def XPluginEnable(self):
        if not os.path.exists(SAMPLE_IMG):
            self.log("Missing sample image, required for this test. Looking for: {}".format(SAMPLE_IMG))
            return 0

        if XPLMMapExists(XPLM_MAP_USER_INTERFACE):
            self.log("map already exists in enable, creating layer immediately")
            self.createOurMapLayer(XPLM_MAP_USER_INTERFACE, None)
        else:
            self.log("map does not exist in enable, will create layer using callback")
    ...
    def createOurMapLayer(self, mapIdentifier, refcon):
        if (not self.g_layer and mapIdentifier == XPLM_MAP_USER_INTERFACE):
            self.log("Map exists, creating layer on {}".format(mapIdentifier))
            self.mapRefCon = []
            self.name = "Markings"
    ...
    def draw_marking_icons(self, layer, inMapBoundsLeftTopRightBottom, zoomRatio,
        ...
        if XPLMGetElapsedTime() - self.lastReported > 5:
            ...
            # self.log("North is {:.1f}".format(north))
    ...
    def prep_cache(self, layer, inTotalMapBoundsLeftTopRightBottom, projection, refcon):
        ...
        self.log("North is {:.1f}".format(self.north))

        self.s_icon_width = XPLMMapScaleMeter(projection, midpoint_x, midpoint_y) * 5000
        self.log('icon-width: {}'.format(self.s_icon_width))

    def will_be_deleted(self, layer, refcon):
        self.log("Delete callback. my layer?{}".format(layer == self.g_layer))
