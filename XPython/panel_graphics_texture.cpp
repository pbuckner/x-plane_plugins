...
static PyObject *XPLMTextureAtlasGetImageWidthFun(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "Oi", keywords, &atlasCapsule, &imageIndex)){
    return nullptr;
  }
  XPLMTextureAtlasRef atlas = getVoidPtr(atlasCapsule, ATLAS_CAPSULE);
...
My_DOCSTR(_textureAtlasGetImageUVMap__doc__, "textureAtlasGetImageUVMap",
          "atlas, imageIndex",
          "atlas:XPLMTextureAtlasRef, imageIndex:int",
          "tuple[float, float, float, float]",
          "Return the UV coordinates (s1, t1, s2, t2) of an image within the atlas\n"
          "texture, where (s1, t1) is the bottom-left corner and (s2, t2) the\n"
          "top-right, in atlas texture space. Useful with textureAtlasDrawMesh().");
static PyObject *XPLMTextureAtlasGetImageUVMapFun(PyObject *self, PyObject *args, PyObject *kwargs)
{
  static char *keywords[] = {CHAR("atlas"), CHAR("imageIndex"), nullptr};
  (void) self;
  PyObject *atlasCapsule;
  int imageIndex;
  if(!XPLMTextureAtlasGetImageUVMap_ptr){
    PyErr_SetString(PyExc_RuntimeError , "XPLMTextureAtlasGetImageUVMap is available only in XPLM440 and up.");
    return nullptr;
  }
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "Oi", keywords, &atlasCapsule, &imageIndex)){
    return nullptr;
  ...
  XPLMTextureAtlasGetImageUVMap_ptr(atlas, imageIndex, uv);
  return Py_BuildValue("(ffff)", uv[0], uv[1], uv[2], uv[3]);
}

My_DOCSTR(_textureAtlasDrawAt__doc__, "textureAtlasDrawAt",
          "atlas, imageIndex, tintColor, x, y",
          ...
          "(x, y). tintColor is multiplied with the texture; use makeColor(1, 1, 1, 1)\n"
          "for no tinting.");
static PyObject *XPLMTextureAtlasDrawAtFun(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "Oikff", keywords, &atlasCapsule, &imageIndex, &tintColor, &x, &y)){
    return nullptr;
  }
  XPLMTextureAtlasRef atlas = getVoidPtr(atlasCapsule, ATLAS_CAPSULE);
...
My_DOCSTR(_textureAtlasDrawIn__doc__, "textureAtlasDrawIn",
          "atlas, imageIndex, tintColor, left, top, right, bottom",
          ...
          "bottom), stretching or compressing to match. tintColor is multiplied\n"
          "with the texture.");
static PyObject *XPLMTextureAtlasDrawInFun(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "Oikffff", keywords, &atlasCapsule, &imageIndex, &tintColor, &left, &top, &right, &bottom)){
    return nullptr;
  }
  XPLMTextureAtlasRef atlas = getVoidPtr(atlasCapsule, ATLAS_CAPSULE);
...
My_DOCSTR(_textureAtlasDrawStretched__doc__, "textureAtlasDrawStretched",
          ...
          "scaling UI elements. tintColor is multiplied with the texture.");
...
My_DOCSTR(_textureAtlasDrawScaled__doc__, "textureAtlasDrawScaled",
          "atlas, imageIndex, tintColor, xPanel, yPanel, xAtlas, yAtlas, xScale, yScale, rotateCW",
          ...
          "is aligned to the panel-space point (xPanel, yPanel), then scaled by\n"
          "(xScale, yScale) and rotated rotateCW degrees clockwise about that pivot.\n"
...
static PyObject *XPLMTextureAtlasDrawScaledFun(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  int imageIndex;
  unsigned long tintColor;
  float xPanel, yPanel, xAtlas, yAtlas, xScale, yScale, rotateCW;
  ...
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "Oikfffffff", keywords, &atlasCapsule, &imageIndex, &tintColor,
...
My_DOCSTR(_textureAtlasDrawMesh__doc__, "textureAtlasDrawMesh",
          "atlas, imageIndex, tintColor, vertices",
          "atlas:XPLMTextureAtlasRef, imageIndex:int, tintColor:int, vertices:Sequence[tuple[float, float, float, float]]",
          "None",
...
static PyObject *XPLMTextureAtlasDrawMeshFun(PyObject *self, PyObject *args, PyObject *kwargs)
{
  ...
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "OikO", keywords, &atlasCapsule, &imageIndex, &tintColor, &verticesObj)){
    return nullptr;
  }
  Py_ssize_t count;
...
My_DOCSTR(_textureSourceDrawIn__doc__, "textureSourceDrawIn",
          "tex, tintColor, left, top, right, bottom",
...
My_DOCSTR(_drawCalls__doc__, "drawCalls",
          ...
          "vertices:bytes, indices:bytes, drawCalls:Sequence",
          "None",
          ...
          "uint16 indices. drawCalls is a sequence of (tex, scissors, idx_offset,\n"
...
{
  static char *keywords[] = {CHAR("vertices"), CHAR("indices"), CHAR("drawCalls"), nullptr};
  (void) self;
  const char *vbuf; Py_ssize_t vlen;
  const char *ibuf; Py_ssize_t ilen;
  PyObject *drawCallsSeq;
  if(!XPLMDrawCalls_ptr){
    PyErr_SetString(PyExc_RuntimeError , "XPLMDrawCalls is available only in XPLM440 and up.");
    return nullptr;
  }
  if(!PyArg_ParseTupleAndKeywords(args, kwargs, "y#y#O", keywords, &vbuf, &vlen, &ibuf, &ilen, &drawCallsSeq)){
    return nullptr;
  }
  if(vlen % 20 != 0){
  ...
  if(ilen % 2 != 0){
    PyErr_SetString(PyExc_ValueError, "drawCalls: indices length must be a multiple of 2 (uint16/index).");
    return nullptr;
  }
  ...
  XPLMMesh_t mesh;
  mesh.vertex_count = (int)(vlen / 20);
  mesh.vertices = (const float *)vbuf;
  mesh.index_count = (int)(ilen / 2);
  mesh.indices = (const uint16_t *)ibuf;
...
PyMethodDef panelGraphicsTextureMethods[] = {
  ...
  {"textureAtlasGetImageUVMap", (PyCFunction)XPLMTextureAtlasGetImageUVMapFun, METH_VARARGS | METH_KEYWORDS, _textureAtlasGetImageUVMap__doc__},
  {"XPLMTextureAtlasGetImageUVMap", (PyCFunction)XPLMTextureAtlasGetImageUVMapFun, METH_VARARGS | METH_KEYWORDS, ""},
